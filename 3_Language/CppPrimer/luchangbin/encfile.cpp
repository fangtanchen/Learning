...
void testAes(char inString[], int inLen, char passwd[], int pwdLen,char op[])
{
	...
	if(strcmp(op,"enc")==0){
		...
		fstream f1;
		f1.open("ciphertext.txt",ios::app);
		f1.write(enString,strlen(enString));
		f1.close();
	}
	// 密文串的解密	
		...
		fstream f2;
		f2.open("plaintext.txt",ios::app);
		f2.write(deString,strlen(deString));
		f2.close();
	}
	//比较解密后的串是否与输入的原始串相同
...
int main(int argc, char* argv[])
{
	...
	while(!f.eof()){
	    ...
	    f1.read(inString,100);
	
	    }
	    //char inString[] = "This is a sample. I am a programer.\n";
	    //char passwd[] = "0123456789ABCDEFGHIJK";
	testAes(inString, strlen(inString), argv[3], strlen(argv[3]),argv[1]);
	f.close();
